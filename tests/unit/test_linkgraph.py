...
def test_resolve_relative_in_same_dir():
    assert resolve_link('guides/install.md', 'configure.md') == (
        'guides/configure.md',
        None,
    )
...
def test_resolve_parent_dir():
    assert resolve_link('guides/install.md', '../concepts/architecture.md') == (
...
def test_resolve_keeps_anchor():
    assert resolve_link('guides/install.md', 'configure.md#prereqs') == (
        'guides/configure.md',
        'prereqs',
    )
...
def test_resolve_strips_query_keeps_anchor():
    assert resolve_link('guides/install.md', 'configure.md?x=1#sec') == (
        'guides/configure.md',
        'sec',
    )
...
def test_extract_sections_strips_trailing_hash_chars():
    ...
    sections = extract_sections(md, [2])
    assert sections == [
        Section(level=2, title='Heading', slug='heading', line_offset=0)
    ]
