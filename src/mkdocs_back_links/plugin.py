...
class BackLinksPlugin(BasePlugin[BackLinksConfig]):
    ...
    def on_files(self, files: Files, config):
        for f in files:
            if (
                f.src_uri.endswith('.md')
                and f.page is None
                and not f.is_documentation_page()
            ):
                # is_documentation_page filters; but be safe
                continue
            if f.src_uri.endswith('.md'):
                self._urls[f.src_uri] = (
                    '/' + f.url if not f.url.startswith('/') else f.url
                )
        return files
                ...
                entries.append(
                    {
                        ...
                        'section_title_lookup': self._section_titles.get(
                            (src_page, src_section)
                        )
                        if src_section
                        else None,
                    }
    ...
    def on_page_context(self, context, *, page: Page, config, nav):
        ...
        backlinks_enabled = self.config.backlinks.enabled and overrides.get(
            'backlinks', True
        )
        ...
        if graph_enabled and not hide_local:
            ...
            graph = {
                ...
                'current_url': self._urls.get(page_id, '/' + page_id),
                'nodes': sorted(
                    nodes_by_id.values(), key=lambda n: (n['type'] != 'page', n['id'])
                ),
                'edges': sorted(
                    edges_by_key.values(),
